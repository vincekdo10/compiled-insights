  ...
  return (
    <section className="border-y border-ci-border bg-ci-surface px-8 py-9">
      <header className="mb-6 text-center">
        <h2 className="text-lg font-bold tracking-tight text-ci-text">
          Who I work with
        </h2>
        <p className="mt-1.5 text-xs text-ci-subtle">
      ...
      <div className="grid grid-cols-1 gap-3.5 md:grid-cols-2">
        <article className="rounded-b-[10px] border border-ci-border border-t-2 border-t-ci-teal bg-ci-deep p-5">
          <div className="mb-3.5 flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-ci-text">
            ...
            {startupItems.map((text) => (
              <li key={text} className="flex items-start gap-2.5">
                <Check
                  ...
                  strokeWidth={2.5}
                />
                <span className="text-xs leading-relaxed text-ci-muted">{text}</span>
              </li>
            ))}
          </ul>
        </article>
